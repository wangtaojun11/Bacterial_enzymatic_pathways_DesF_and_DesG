...
```{r}
d1 <- read_csv("LNCaP_proliferation_ATT.csv") 
head(d1)
```
...
```{r}
d1$Androgens = factor(d1$Androgens,order = TRUE, levels = c("CL", "AT", "ATT"))
```
...
p_ATT <- ggplot(d1_summary, aes(x = Androgens, y = Relative_cell_proliferation, fill = Enzalutamide)) +
...
ggsave("LNCaP_proliferation_ATT.pdf", height = 3.5, width = 4.5)
...
d1_summary_ATT <- subset(d1_summary, Androgens == "ATT")
pairwise.t.test(d1_summary_ATT$Relative_cell_proliferation, d1_summary_ATT$Enzalutamide, p.adjust.method = "BH")
